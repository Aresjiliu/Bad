...
def deeppix_main(args):
    ...
    args.p = [1, 1]
    teacher_model = HSI_Lidar_Couple_Share(args, modality_1_channel, modality_2_channel)

    args.p = [0, 1]
    student_model = HSI_Lidar_Couple_Share(args, modality_1_channel, modality_2_channel)
    ...
    if 'lidar' in args.pair_modalities:
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root,
                                    'share_unimodal_center_full/hsi_lidar_7x7_couple_cross_fc_version_2_labma_unimodal3.0' + '.pth')))
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root,
                                    'share_unimodal_center_full/hsi_ms_7x7_couple_cross_fc_version_2_labma_unimodal0.5.pth')))

    for param in teacher_model.parameters():
        param.requires_grad = False

    args.model_root = os.path.join(args.model_root, 'share_center_transfer_lidar' + str(args.patch_size) + 'x' + str(
        args.patch_size),)
    args.log_root = os.path.join(args.log_root, 'share_center_transfer_lidar' + str(args.patch_size) + 'x' + str(
        args.patch_size))
