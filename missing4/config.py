...
def get_args():
    ...
    # FMC模块参数
    parser.add_argument('--use_fmc', type=bool, default=True, help='是否使用FMC模块')
    parser.add_argument('--fmc_epochs', type=int, default=100, help='FMC模块训练轮数')
    parser.add_argument('--feature_dim', type=int, default=256, help='特征维度')
    parser.add_argument('--fmc_lr', type=float, default=0.0002, help='FMC模块学习率')
    parser.add_argument('--fmc_beta1', type=float, default=0.5, help='FMC模块Adam优化器beta1')
    parser.add_argument('--fmc_beta2', type=float, default=0.999, help='FMC模块Adam优化器beta2')
    parser.add_argument('--fmc_recon_weight', type=float, default=0.1, help='重构损失权重')
    parser.add_argument('--fmc_model_path', type=str, default='../output/models/fmc/', help='FMC模型保存路径')
    parser.add_argument('--hsi_channels', type=int, default=144, help='HSI通道数')
    parser.add_argument('--radar_channels', type=int, default=1, help='雷达通道数')
    parser.add_argument('--image_size', type=int, default=7, help='图像尺寸')
    ...
    # FMC相关路径
    args.output_dir = args.fmc_model_path + str(args.identity)
    args.model_dir = args.model_root

    # 创建FMC相关目录
    os.makedirs(args.fmc_model_path, exist_ok=True)
