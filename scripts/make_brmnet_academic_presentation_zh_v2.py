...
OUT = ROOT / "docs" / "slides" / "BRM-Net_Academic_Presentation_ZH_v2.pptx"
...
def dark_box(s, x, y, w, h, title, body, accent=CYAN):
    ...
    text(s, body, x + 0.22, y + 0.68, w - 0.36, h - 0.78, 17, LIGHT)
...
def chart(s, cats, series, x, y, w, h, title=None):
    ...
    ch.value_axis.tick_labels.font.size = Pt(14)
...
def build(output: Path) -> None:
    ...
    dark_box(s, 4.55, 3.55, 3.25, 1.25, "核心证据", "目标 MAC 对齐、真实导出、多状态鲁棒性。", ORANGE)
    dark_box(s, 8.25, 3.55, 3.25, 1.25, "汇报定位", "18 页主汇报 + 4 页备份页。", GREEN)
    text(s, "毕业预答辩 / 学术组会 / 投稿汇报", 0.85, 6.25, 6.0, 0.3, 17, LIGHT)
    ...
    text(s, "LiDAR / Aux", 4.0, 1.75, 2.3, 0.4, 30, GREEN, True)
    text(s, "几何/高度信息\n提升类别区分", 4.0, 2.35, 2.4, 0.8, 21, BLACK)
    text(s, "=>", 6.75, 2.25, 0.6, 0.35, 30, CYAN, True)
    text(s, "多分支融合", 7.65, 1.75, 2.4, 0.4, 30, NAVY, True)
    text(s, "精度提升\n计算和故障模式增加", 7.65, 2.35, 3.0, 0.8, 21, BLACK)
    box(s, 1.0, 4.3, 10.6, 1.0, "部署导向评价", "不能只看 full-modality OA，还要报告真实 MAC、参数、缺失模态和退化模态表现。", ORANGE, body_size=20)
    ...
    box(s, 2.0, 1.35, 9.4, 1.0, "融合兼容预算化紧凑导出", "学习通道门控，并在保持融合接口的前提下导出物理紧凑模型。", CYAN, body_size=20)
    text(s, "2", 1.2, 3.0, 0.7, 0.5, 42, ORANGE, True)
    box(s, 2.0, 2.8, 9.4, 1.0, "退化监督可靠性融合", "显式监督 corrupted-but-available 输入，不依赖重建网络。", ORANGE, body_size=20)
    text(s, "3", 1.2, 4.45, 0.7, 0.5, 42, GREEN, True)
    box(s, 2.0, 4.25, 9.4, 1.0, "精度-效率-鲁棒性联合评估", "报告 actual MAC、missing states、degraded states 和多数据集证据。", GREEN, body_size=20)
    takehome(s, "投稿主线收束为三点：真实导出、退化可靠融合、联合评估。")
    footer(s, 5)

    s = slide(prs, "BRM-Net 总体框架：双分支、三阶段", "05 方法")
    text(s, "Training", 0.85, 1.2, 1.4, 0.3, 20, CYAN, True)
    text(s, "Compact Export", 5.25, 1.2, 2.1, 0.3, 20, ORANGE, True)
    text(s, "Inference", 9.6, 1.2, 1.4, 0.3, 20, GREEN, True)
    for y, name, color in [(1.8, "HSI", BLUE), (3.2, "Aux", GREEN)]:
        box(s, 0.75, y, 1.55, 0.75, name, "", color, body_size=18)
        box(s, 2.55, y, 1.5, 0.75, "Enc", "", color, body_size=18)
        box(s, 4.35, y, 1.25, 0.75, "Gate", "", CYAN, body_size=18)
        box(s, 6.0, y, 1.7, 0.75, "Q head", "", ORANGE, body_size=18)
        line(s, 2.3, y + 0.38, 2.55, y + 0.38)
        line(s, 4.05, y + 0.38, 4.35, y + 0.38)
        line(s, 5.6, y + 0.38, 6.0, y + 0.38)
    box(s, 8.0, 2.45, 1.8, 0.85, "Fusion", "", GREEN, body_size=18)
    box(s, 10.25, 2.45, 1.35, 0.85, "Head", "", NAVY, body_size=18)
    line(s, 7.7, 2.18, 8.0, 2.7)
    line(s, 7.7, 3.58, 8.0, 3.0)
    line(s, 9.8, 2.88, 10.25, 2.88)
    box(s, 4.3, 4.8, 2.2, 0.75, "阈值 η 选择", "", ORANGE, body_size=18)
    box(s, 6.95, 4.8, 2.5, 0.75, "依赖通道删除", "", ORANGE, body_size=18)
    box(s, 9.9, 4.8, 2.0, 0.75, "紧凑模型", "", GREEN, body_size=18)
    line(s, 5.6, 4.0, 5.4, 4.8, ORANGE, True)
    line(s, 6.5, 5.18, 6.95, 5.18, ORANGE)
    line(s, 9.45, 5.18, 9.9, 5.18, ORANGE)
    ...
    bullets(s, ["门控只用于学习通道重要性。", "最终结论必须基于导出的紧凑模型。"], 7.1, 4.35, 4.6, 1.1, 21)
    takehome(s, "实际 MAC 与 65% / 80% / 90% 目标高度一致。")
    footer(s, 7)

    s = slide(prs, "融合兼容 compact export：从 source 到 compact", "07 方法")
    box(s, 0.9, 1.65, 2.5, 1.2, "Source gated model", "保留完整网络\n门控仍在源结构中", CYAN, body_size=18)
    line(s, 3.4, 2.25, 4.25, 2.25, ORANGE)
    box(s, 4.25, 1.65, 2.4, 1.2, "Gate projection", "阈值选择\n终端兼容约束", ORANGE, body_size=18)
    line(s, 6.65, 2.25, 7.5, 2.25, ORANGE)
    box(s, 7.5, 1.65, 2.5, 1.2, "Physical removal", "删除通道、BN\n和依赖切片", ORANGE, body_size=18)
    line(s, 10.0, 2.25, 10.8, 2.25, GREEN)
    box(s, 10.8, 1.65, 1.7, 1.2, "Compact", "可部署", GREEN, body_size=18)
    table(s, ["状态", "Source OA", "Compact OA", "变化"], [
        ["Full", "86.15", "86.90", "+0.75"],
        ["HSI only", "78.18", "80.04", "+1.86"],
        ["Occlusion-50", "84.59", "85.57", "+0.98"],
    ], 1.0, 4.05, 10.7, 1.45, 16)
    ...
    s = slide(prs, "退化监督可靠性：让模型见到 corrupted-but-available 输入", "09 方法")
    ...
    box(s, 8.75, 1.65, 3.1, 1.2, "读图方式", "横轴为实际 MAC\n纵轴为 OA", CYAN, body_size=20)
    box(s, 8.75, 3.25, 3.1, 1.2, "关键点", "80% 目标附近\n比较 compact accuracy", ORANGE, body_size=20)
    ...
    text(s, "74.89 → 84.76", 8.95, 2.05, 2.8, 0.45, 30, ORANGE, True, PP_ALIGN.CENTER)
    text(s, "adverse-state average", 8.75, 2.6, 3.1, 0.25, 18, GRAY, align=PP_ALIGN.CENTER)
    ...
    box(s, 0.95, 3.75, 3.45, 1.25, "已支撑", "预算对齐、真实导出、退化鲁棒性。", GREEN, body_size=20)
    box(s, 4.85, 3.75, 3.45, 1.25, "需谨慎", "MUUFL 存在 clean-robustness trade-off。", ORANGE, body_size=20)
    box(s, 8.75, 3.75, 3.45, 1.25, "下一步", "更大 backbone、更多模态组合、更密集退化曲线。", CYAN, body_size=20)
