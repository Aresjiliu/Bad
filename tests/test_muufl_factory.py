...
from brmnet_core.data.muufl import MUUFLScene


class MUUFFactoryTest(unittest.TestCase):
    def test_builds_muufl_loaders_from_fixed_split_file(self):
        ...
        self.assertEqual(batch["m_1"].shape[1:], (64, 3, 3))
        self.assertEqual(batch["m_2"].shape[1:], (2, 3, 3))
        self.assertEqual(bundle.metadata["dataset"], "muufl")
        self.assertEqual(bundle.metadata["train_samples"], 2)
        self.assertEqual(bundle.metadata["test_samples"], 2)
        self.assertEqual(bundle.metadata["aux_channel_mode"], "both")
