...
class TorchTransformer(nn.Module):
    ...
    def summary(self, model=None, input_tensor=None):
        ...
        # loop graph
        print("##########################################################################################")
        line_title = "{:>5}| {:<15} | {:<15} {:<25} {:<15}".format("Index", "Layer (type)", "Bottoms", "Output Shape",
                                                                   "Param #")
        print(line_title)
        print("---------------------------------------------------------------------------")
        ...
        for layer_index, key in enumerate(model_graph):
            ...
            if bottoms_graph[key] is None:
                ...
                new_layer = "{:5}| {:<15} | {:<15} {:<25} {:<15}".format(layer_index, layer_type, "", output_shape,
                                                                         param_weight_num)
                print(new_layer)
                ...
                for idx, b in enumerate(bottoms):
                    ...
                    if idx == 0:
                        new_layer = "{:>5}| {:<15} | {:<15} {:<25} {:<15}".format(layer_index, layer_type, b,
                                                                                  output_shape, param_weight_num)
                    else:
    ...
    def _build_graph(self, model, input_tensor=None):
        ...
        for f in dir(torch):
            ...
            if f.startswith("_") or "tensor" == f:
                continue
            if isinstance(getattr(torch, f), types.BuiltinMethodType) or isinstance(getattr(torch, f),
        ...
        for f in dir(F):
            ...
            if isinstance(getattr(F, f), types.BuiltinMethodType) or isinstance(getattr(F, f),
                                                                                types.BuiltinFunctionType) or isinstance(
                    getattr(F, f), types.FunctionType):
    ...
    def _trans_unit(self, model):
        ...
        for module_name in model._modules:
            ...
            if len(model._modules[module_name]._modules) > 0 and \
                    not (len(model._modules[module_name]._modules) == 1 and type(
                        list(model._modules[module_name]._modules.values())[0]) == QuantMeasure):
    ...
    def _torchFunctions(self, raw_func, *args, **kwargs):
        ...
        if isinstance(logs, tuple) or isinstance(logs, list) or isinstance(out_tensor, tuple) or (
                logs.cur_tensor.size() != out_tensor.size()):
            ...
            cur_log.record_tensor_op.append(
                'torch_{}_{}_{}'.format(function_name, inspect.stack()[2].lineno, len(bottoms)))
