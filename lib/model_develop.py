...
from models.base_model import Conv2d_Prune
...
def calc_accuracy_multi_first(model, loader, args, verbose=False, hter=False):
    """
    :param model: model network
    :param loader: torch.utils.data.DataLoader
    :param verbose: show progress bar, bool
    ...
    for batch_sample in tqdm(iter(loader), desc="Full forward pass", total=len(loader), disable=not verbose):
        ...
        if torch.cuda.is_available():
            img_m1 = img_m1.cuda()
            img_m2 = torch.zeros_like(img_m2).cuda()
            target = target.cuda()
...
def calc_accuracy_multi_second(model, loader, args, verbose=False, hter=False):
    """
    :param model: model network
    :param loader: torch.utils.data.DataLoader
    :param verbose: show progress bar, bool
    ...
    for batch_sample in tqdm(iter(loader), desc="Full forward pass", total=len(loader), disable=not verbose):
        ...
        img_m1, img_m2, target = batch_sample['m_1'], batch_sample['m_2'], \
            batch_sample['label']
        if torch.cuda.is_available():
            img_m1 = torch.zeros_like(img_m1).cuda()
            img_m2 = img_m2.cuda()
            target = target.cuda()
...
def train_base_multi_share_unimodal_center_prune(model, cost, optimizer, train_loader, test_loader, args):
    '''
    适用于多模态分类的基础训练函数
    :param model:
    :param cost:
    ...
    while epoch < epoch_num:
        for batch_idx, batch_sample in enumerate(
            ...
            output, hsi_out, lidar_out = model(img_hsi, img_lidar)
            assert torch.isnan(output).sum() == 0, print(output)
            cls_loss1 = cost(output, target)
            cls_loss2 = cost(hsi_out, target)
            cls_loss3 = cost(lidar_out, target)
            cls_loss = cls_loss1
            unimodal_loss = mse_func(hsi_out, lidar_out) + cls_loss2 + cls_loss3
            l1_loss = model.l1_loss(epoch)
            total_loss = cls_loss + unimodal_loss * args.labma_unimodal + l1_loss * args.l1_loss
        ...
        model.update_temp()
        log_list.append(train_loss / len(train_loader))
        log_list.append(unimodal_loss_sum / len(train_loader))
        log_list.append(cls_sum / len(train_loader))
        log_list.append(l1_loss_sum / len(train_loader))
        log_list.append(accuracy_test)
        log_list.append(result_test[1])
        log_list.append(result_test[2])
        log_list.append(accuracy_best)
        print("Epoch {}, cls_loss={:.5f},rec_loss={:.5f}, accuracy_test={:.5f},  accuracy_best={:.5f}"
              .format(epoch, train_loss / len(train_loader), rec_loss_sum / len(train_loader), accuracy_test,
                      accuracy_best))
...
def train_knowledge_distill_fc_feature_shaspec(net_dict, cost_dict, optimizer, train_loader, test_loader, args):
    ...
    from loss.kd.pkt import PKTCosSim
    from loss.kd.sp import SP, DAD, DGD
    from loss.kd.at import AT
