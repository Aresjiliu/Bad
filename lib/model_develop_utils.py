...
def train_base(model, cost, optimizer, train_loader, test_loader, args):
    ...
    while epoch < epoch_num:
        ...
        # testing
        result_test = calc_accuracy(model, loader=test_loader, verbose=True)
...
def modality_drop(data, p, args):
    if len(p) == 2:
        ...
        if args.drop_mode == 'average':
            prob = np.array((1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7, 1 / 7))
        elif args.drop_mode == 'unaverage':
            prob = np.array((1 / 4, 1 / 4, 1 / 4, 0, 0, 0, 1 / 4))
        else:
            raise Exception('invalid drop mode')
    else:
        pass

    if torch.sum(torch.tensor(p)) == 0:
        p = []
        ...
        p = [p * data[0].shape[0]]
        # print(p)
        p = np.array(p).reshape(data[0].shape[0], len(data))
        p = torch.from_numpy(p)
        p = torch.unsqueeze(p, 2)
        p = torch.unsqueeze(p, 3)
        p = torch.unsqueeze(p, 4)
