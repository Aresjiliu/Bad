...
class SFFModule(nn.Module):
    ...
    def forward(self, F_sh_V, F_sp_V, F_sh_I, F_sp_I, labels):
        ...
        return S_fp_V, S_fp_I, identity_loss
...
class UnifiedFMCNet(nn.Module):
    ...
    def forward(self, x_hs, x_radar, labels):
        ...
        f_fp_hs, f_fp_radar, identity_loss = self.sff(
            f_sh_hs, f_sp_hs, f_sh_radar, f_sp_radar, labels
        )
        full, full_loss = self.full(f_sh_hs, f_sh_radar, f_sp_hs, f_sp_radar, labels)

        total_loss = identity_loss + md_loss + id_loss + full_loss
        losss = [md_loss, id_loss, identity_loss, full_loss]
