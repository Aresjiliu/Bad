...
def deeppix_main(args):
    ...
    if 'lidar' in args.pair_modalities:
        ...
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'shaspec_full/hsi_lidar_7x7_couple_cross_fc_version_0_labma_unimodal1.0' + '.pth')))
        teacher_model.eval()
    elif 'ms'  in args.pair_modalities:
        teacher_model.load_state_dict(
            torch.load(os.path.join(args.model_root, 'shaspec_full/hsi_ms_7x7_couple_cross_fc_version_0_labma_unimodal1.0.pth')))
        teacher_model.eval()
    else:
    ...
    args.model_root = os.path.join(args.model_root,'share_center_1.5_transfer01', 'kd_'+args.location+'pooling', 'margin_' + str(args.margin))
    args.log_root = os.path.join(args.log_root, 'share_center_1.5_transfer01','kd_'+args.location+'pooling', 'margin_' + str(args.margin))
