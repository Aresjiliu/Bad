...
- The canonical evidence matrix is now generated. It confirms that core compact-export, source-vs-compact, degradation-robustness, and multi-dataset evidence are ready, while strict uniform-width, w/o availability mask, and w/o terminal-tie ablations remain either missing or require rewording.
- The source-vs-compact table closes the soft-mask-only narrative gap for the current submission: the source gated model and exported compact model are reported separately under the same 80% multi-degradation setting, and compact performance remains close across full, missing, and degraded states.
- The w/o availability-mask gap is now closed as a fusion diagnostic. `--disable-fusion-availability-mask` removes only the fusion softmax mask and keeps availability-conditioned branch execution. Three-seed Houston2013 results show the main-only compact OA drops from 80.04 +/- 1.93 to 76.72 +/- 2.97, while full and degraded-but-available states remain close. This supports a precise claim: fusion availability masking is most important for physically missing modalities, not for ordinary corruption.

## 2026-07-18 uniform-width export findings

- The strict target-matched uniform-width export gap is now closed for the submission scope.
- On Houston2013 three seeds, learned nonuniform export reaches 86.90 +/- 1.65 full OA at about 80% MACs, while uniform-width export reaches 86.29 +/- 0.77. The full-modality gap is modest, so this ablation should not be overstated as a large clean-accuracy win.
