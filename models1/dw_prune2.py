...
class MCL_Fusion(nn.Module):
    def __init__(self, in_channels, osc):
        super().__init__()
        self.block = DepthwiseSeparableConv1(in_channels * 2, in_channels, kernel_size=1, stride=1, padding=0, osc=osc)
...
class MDMB_fusion_share(nn.Module):
    def __init__(self, input_channel, args):
        super().__init__()
        self.block_5_1 = DepthwiseSeparableConv1(input_channel, 128, kernel_size=3, stride=1, padding=1, osc=args.osc)

        self.block_5_2 = DepthwiseSeparableConv1(128, 64, kernel_size=3, stride=1, padding=1, osc=args.osc)
